    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            ...
            while (true) {   // ciclo al infinito para elfuncionamiento del server
                ...
                String[] nums = cadena.split(","); //separamos números por comas
                for (int i = 0; i < nums.length; i++) {
                    int num = Integer.parseInt(nums[i].trim()) + 5; // +5 a cada número
                    nums[i] = String.valueOf(num);
                }
                cadena = String.join(", ", nums); //juntamos los números con comas

                toClient = new PrintStream(client.getOutputStream()); //prepara el objeto para enviar
